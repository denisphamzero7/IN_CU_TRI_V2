...
class MidPanelView(ttk.Frame):
    def __init__(self, parent, router):
        ...
        # Nhãn "Đã chọn" (Trái)
        self.lbl_count = ttk.Label(footer_frame, text="Đã chọn: 0 người", font=("Segoe UI", 10, "italic"), bootstyle="danger")
    ...
    def _setup_toolbar(self):
        ...
        self.search_view.grid(row=0, column=0, columnspan=2, sticky="w")

        create_button(toolbar, "☑ All", self.router.select_all, style="primary-outline", width=6).grid(row=0, column=4, padx=2)
        create_button(toolbar, "☐ Bỏ", self.router.deselect_all, style="secondary-outline", width=6).grid(row=0, column=5, padx=2)

        self.cbb_filter = ttk.Combobox(toolbar, state="readonly", justify="left", width=15) 
        self.cbb_filter.grid(row=0, column=6, padx=10)
        self.cbb_filter.set("Lọc khu vực...") 
        self.cbb_filter.bind("<<ComboboxSelected>>", self.router.on_filter_change)

        fr_page = ttk.Frame(toolbar)
        fr_page.grid(row=0, column=7, sticky="e")
    ...
    def update_data(self, df, custom_configs):
        ...
        if df is None or df.empty:
            self.lbl_placeholder.place(relx=0.5, rely=0.5, anchor="center")
            self.lbl_total.config(text="Tổng số: 0")
            return
        else:
            self.lbl_placeholder.place_forget()
            # Không cập nhật lbl_total ở đây để tránh ghi đè logic tìm kiếm/lọc
            pass 
    ...
    def set_total_count(self, count):
        self.lbl_total.config(text=f"Tổng số: {count}")