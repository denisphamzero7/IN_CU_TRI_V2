...
from config.settings import APP_BG_COLOR, MISA_BORDER_COLOR, MISA_WHITE
...
class RightPanelView(ttk.Frame):
    def __init__(self, parent, router):
        ...
        style.configure('MisaToolbar.TLabel', background=APP_BG_COLOR, foreground="#333333", font=("Segoe UI", 9, "bold"))
        style.configure('Toolbar.TButton', font=("Segoe UI", 9))
        
        # Checkbox trên Toolbar (Nền xanh nhạt)
        ...
        # --- 3. CANVAS ---
        self.canvas = tk.Canvas(self, bg="#57606f", cursor="fleur", highlightthickness=0, takefocus=1)
    ...
    def _setup_top_toolbar(self):
        # [SỬA LỖI] Đặt tên là self.tb_frame để khớp với MainView
        self.tb_frame = ttk.Frame(self, padding=(10, 5), style='Misa.TFrame')
        self.tb_frame.pack(fill=X, side=TOP)

        # ============================================================
        # DÒNG 1: CẤU HÌNH GIẤY & IN ẤN
        # ============================================================
        # row1 là con của self.tb_frame
        row1 = ttk.Frame(self.tb_frame, style='Misa.TFrame')
        row1.pack(fill=X, side=TOP, pady=(0, 5))
        ...
        cbb_size = ttk.Combobox(fr_paper, textvariable=self.var_paper_size, values=["A4", "A5", "A6", "TheCuTri"], width=8, state="disabled", bootstyle="primary") 
        cbb_size.pack(side=LEFT, padx=(2, 10))
        cbb_size.bind("<<ComboboxSelected>>", self.router.on_paper_config_change)

        ttk.Label(fr_paper, text="Hướng:", style='MisaToolbar.TLabel').pack(side=LEFT)
        self.var_orientation = tk.StringVar(value="Ngang")
        self.cbb_orientation = ttk.Combobox(fr_paper, textvariable=self.var_orientation, values=["Dọc", "Ngang"], width=8, bootstyle="primary",state="disabled") 
        self.cbb_orientation.pack(side=LEFT, padx=(2, 10))
        self.cbb_orientation.bind("<<ComboboxSelected>>", self.router.on_orientation_change)

        self.btn_rotate = ttk.Button(fr_paper, text="↻ Xoay Ảnh", command=self.router.rotate_template_right, bootstyle="secondary-outline", style="Toolbar.TButton", width=10, state="disabled")
        self.btn_rotate.pack(side=LEFT, padx=5)

        # --- Nhóm Phải (In ấn) ---
        create_button(row1, "🖨️ IN NGAY", self.router.start_print, style="danger").pack(side=RIGHT, padx=(10, 0))
        
        self.var_print_mode = tk.StringVar(value="Chỉ dữ liệu")
        self.cbb_print_mode = ttk.Combobox(row1, textvariable=self.var_print_mode, values=["Chỉ dữ liệu", "Dữ liệu + Phôi"], state="disabled", width=13, bootstyle="info")
        ...
        self.cbb_printer = ttk.Combobox(row1, values=printers, state="disabled", width=25, bootstyle="primary")
        ...
        fr_range.pack(side=RIGHT, padx=(10, 10))
        ttk.Label(fr_range, text="Từ:", style='MisaToolbar.TLabel').pack(side=LEFT)
        self.var_print_from = tk.StringVar(value="1")
        self.entry_from = ttk.Spinbox(fr_range, from_=1, to=9999, textvariable=self.var_print_from, width=5, bootstyle="secondary")
        self.entry_from.pack(side=LEFT, padx=(2, 5))
        ttk.Label(fr_range, text="Đến:", style='MisaToolbar.TLabel').pack(side=LEFT)
        self.var_print_to = tk.StringVar(value="")
        self.entry_to = ttk.Spinbox(fr_range, from_=1, to=9999, textvariable=self.var_print_to, width=5, bootstyle="secondary")
        self.entry_to.pack(side=LEFT, padx=(2, 0))

        # ============================================================
        # DÒNG 2: TÙY CHỈNH STYLE (Nằm trong self.tb_frame luôn)
        # ============================================================
        self.fr_style_toolbar = ttk.Labelframe(self.tb_frame, text="Tùy chỉnh Style đối tượng", style='Misa.TFrame', padding=5)
        self.fr_style_toolbar.pack(fill=X, side=TOP)

        # Container chứa các thuộc tính TEXT
        ...
        self.spin_size = ttk.Spinbox(self.fr_text_props, from_=5, to=300, width=4, command=self.router.on_prop_change)
        ...
        ttk.Checkbutton(self.fr_text_props, text="Đậm (B)", variable=self.chk_bold_var, style="Toolbar.TCheckbutton", command=self.router.on_prop_change).pack(side=LEFT, padx=(0, 10))
        ttk.Checkbutton(self.fr_text_props, text="IN HOA", variable=self.chk_upper_var, style="Toolbar.TCheckbutton", command=self.router.on_prop_change).pack(side=LEFT, padx=(0, 10))

        # -- Color --
        ttk.Label(self.fr_text_props, text="Màu:", style='MisaToolbar.TLabel').pack(side=LEFT, padx=(0,2))
        self.combo_color = ttk.Combobox(self.fr_text_props, values=["Black", "Red", "Blue", "#2c3e50", "#e74c3c", "#16a085", "#8e44ad"], width=10, state="readonly")
        self.combo_color.pack(side=LEFT)
        self.combo_color.bind("<<ComboboxSelected>>", self.router.on_prop_change)

        # Container chứa các thuộc tính IMAGE (Ẩn mặc định)
        ...
        self.spin_img_w = ttk.Spinbox(self.fr_img_props, from_=1, to=1000, width=5, command=self.router.on_prop_change)
        ...
        self.spin_img_h = ttk.Spinbox(self.fr_img_props, from_=1, to=1000, width=5, command=self.router.on_prop_change)
        self.spin_img_h.pack(side=LEFT)
        self.spin_img_h.bind("<Return>", self.router.on_prop_change)

        ttk.Button(self.fr_img_props, text="📂 Chọn file chữ ký khác...", command=self.router.pick_manual_signature, bootstyle="warning-outline", style="Toolbar.TButton").pack(side=LEFT, padx=(20, 0))

        # Nút Reset
        ttk.Button(self.fr_style_toolbar, text="↺ Mặc định", command=self.router.reset_current_custom, bootstyle="link").pack(side=RIGHT)
